...
export const insert = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        for (const [filterKey, filterDoc] of Object.entries(args.document.filterKeys)) {
            ...
            for (const filterValue of valueArray) {
                ...
                await approximateCounter.increment(ctx, pointId, filterCounterKey(filterKey, filterValue));
            }
        }
    },
});
...
function s2Cells(s2, point, opts) {
...
async function removePointByKey(ctx, s2, key, opts) {
    ...
    for (const cell of cells) {
        const existingCell = await ctx.db
            ...
            .unique();
        if (!existingCell) {
            throw new Error(`Invariant failed: Missing cell ${cell} for point ${existing._id}`);
        }
    ...
    for (const [filterKey, filterDoc] of Object.entries(existing.filterKeys)) {
        ...
        for (const filterValue of valueArray) {
            const existingFilterKey = await ctx.db
                ...
                .withIndex("filterKey", (q) => q
                .eq("filterKey", filterKey)
                .eq("filterValue", filterValue)
                .eq("tupleKey", tupleKey))
                .unique();
            if (!existingFilterKey) {
                throw new Error(`Invariant failed: Missing filterKey ${filterKey}:${filterValue} for point ${existing._id}`);
            }
            await ctx.db.delete(existingFilterKey._id);
            await approximateCounter.decrement(ctx, existing._id, filterCounterKey(filterKey, filterValue));
        }
    }
