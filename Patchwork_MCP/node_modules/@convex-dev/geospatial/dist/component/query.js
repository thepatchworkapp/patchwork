...
export const debugCells = query({
    ...
    handler: async (ctx, args) => {
        ...
        const cells = s2.coverRectangle(args.rectangle, args.minLevel, args.maxLevel, args.levelMod, args.maxCells);
...
export const execute = query({
    ...
    handler: async (ctx, args) => {
        ...
        if (sorting.interval.startInclusive !== undefined &&
        ...
        const cells = s2
            .coverRectangle(rectangle, args.minLevel, args.maxLevel, args.levelMod, args.maxCells)
        ...
        const cellRanges = cells.map((cell) => new CellRange(ctx, logger, cell, args.cursor, sorting.interval, PREFETCH_SIZE, stats));
        ...
        for (const filter of args.query.filtering) {
            ...
            ranges.push(new FilterKeyRange(ctx, logger, filter.filterKey, filter.filterValue, args.cursor, sorting.interval, PREFETCH_SIZE, stats));
        }
        // Fourth, build up the final query stream.
        const intersectionStreams = [cellStream];
            ...
            try {
                for await (const { tupleKey, docPromise } of channel) {
                    ...
                    if (results.length >= args.query.maxResults) {
                        logger.debug(`Consumer reached max results of ${args.query.maxResults} at ${tupleKey}`);
                        nextCursor = tupleKey;
                        return;
                    }
                    if (stats.rowsRead >= 1024) {
                        logger.warn(`Consumer reached Convex query limit of 1024 rows at ${tupleKey}`);
                        nextCursor = tupleKey;
                        return;
                    }
                }
...
export const nearestPoints = query({
    ...
    handler: async (ctx, args) => {
        ...
        const query = new ClosestPointQuery(s2, logger, args.point, args.maxDistance, args.maxResults, args.minLevel, args.maxLevel, args.levelMod, args.filtering, args.sorting.interval);
